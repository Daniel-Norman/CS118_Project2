...
int main(int argc, char *argv[]) {
    if (argc < 4) {
	printf("Error: not all fields provided. error rate, timeout, windowsize");
	exit(1);
    }
    ...
    if (fp != NULL) {
        ...
        int window_size = atoi(argv[3]); //TODO: To be passed in
        ...
        int time_out = atoi(argv[2]); //TODO: to be passed in - will need to change if TA wants smaller time interval
